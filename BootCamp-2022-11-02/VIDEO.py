...
#получаем значение с видеокамеры
cap = cv2.VideoCapture(0)

#вечный цикл вывода картинки с камеры
while True:
    #получаем значение с камеры в переменную frame с помощью метода read
    success, frame = cap.read()
    #success, frame = img.read()

    #вывод изображения до обработки нейронной сетью
    ...
    cap_grey = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
    ...
    for (x, y, w, h) in faund_faces:
        #"рисуем" прямоугольник обозначающий лицо 
        cv2.rectangle(frame, (x, y), (x + w, y + h), (0, 0, 255), 2)

    #выводим "отредактированное" изображение
    cv2.imshow('after processing', frame)

    #выходим из цикла нажатием клавиши "q"
    # 1 - время обновления кадра 
